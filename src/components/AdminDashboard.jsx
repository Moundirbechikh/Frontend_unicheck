...
import {
  ...
  Upload, FileText, CheckCircle2, Loader2, Plus, Trash2, ChevronDown
} from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  // ── Cours avec assignations ───────────────────────────────────────────────
  const [newCours, setNewCours] = useState({ libelle: '', specialite: 'SITW' });
  const [assignations, setAssignations] = useState([]);  // [{profId, typeSeance}]
  ...
  // Charger les profs quand la modale cours s'ouvre
  useEffect(() => {
  ...
  // 4. NOUVEAU USE EFFECT POUR CHARGER LES STATS AU DÉMARRAGE
  useEffect(() => {
    fetch('https://backend-unicheck.onrender.com/api/admin/dashboard/stats', { headers: getAuthHeaders() })
      .then(r => r.ok ? r.json() : null)
      .then(data => {
  ...
  // ── Handler cours avec assignations ──────────────────────────────────────
  const addAssignationRow = () => {
    setAssignations(prev => [...prev, { profId: '', typeSeance: 'COURS' }]);
  };

  const updateAssignation = (index, field, value) => {
    setAssignations(prev => prev.map((a, i) =>
      i === index ? { ...a, [field]: value } : a
    ));
  };

  const removeAssignation = (index) => {
      ...
      const res = await fetch("https://backend-unicheck.onrender.com/api/cours/add", {
        ...
        body: JSON.stringify({
          ...
          assignations: assignations.map(a => ({
            profId:   parseInt(a.profId),
            typeSeance: a.typeSeance
          }))
        })
      });
...
{/* Stats bento AVEC LES VRAIES VALEURS */}
<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <motion.div variants={item} className="bento-card flex flex-col justify-between group hover:-translate-y-1 transition-transform">
            ...
            <div>
              ...
              <h3 className="font-display font-black text-[#1a1c1e] text-3xl tracking-tighter">
                {globalStats.totalEtudiants}
              </h3>
            </div>
          </motion.div>
  ...
  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
    {[
      ...
      <button key={label}
        ...
        <div className={`w-12 h-12 bg-[#f1f4f2] group-hover:${hover} rounded-[1.2rem] flex items-center justify-center transition-colors`}>
...
<motion.div variants={item} className="md:col-span-5 space-y-4 flex flex-col">
  ...
  <div className="bg-white p-5 rounded-[2rem] shadow-sm border border-white flex-1 flex flex-col gap-3">
    {[
      { icon: TrendingDown, bg: 'bg-orange-50', ibg: 'bg-orange-100', ic: 'text-orange-500', bc: 'border-orange-100',
        title: 'Baisse de présence (G2)', body: 'Le groupe G2 SITW a une moyenne de 65% cette semaine.' },
      { icon: TrendingUp,   bg: 'bg-[#d1f4e0]/50', ibg: 'bg-[#d1f4e0]', ic: 'text-[#006c49]', bc: 'border-[#006c49]/10',
        title: 'SITW en hausse', body: 'La spécialité SITW atteint 91% de présence ce mois-ci.' },
      { icon: Activity,     bg: 'bg-blue-50/50', ibg: 'bg-blue-100', ic: 'text-blue-500', bc: 'border-blue-100',
        title: '3 justificatifs en attente', body: 'Des absences non justifiées requièrent une attention.' },
    ].map(({ icon: Icon, bg, ibg, ic, bc, title, body }) => (
      <div key={title} className={`flex gap-4 p-3 ${bg} rounded-[1.5rem] border ${bc}`}>
        <div className={`w-10 h-10 ${ibg} ${ic} rounded-[1rem] flex items-center justify-center shrink-0`}>
          <Icon size={18} strokeWidth={3} />
        </div>
        <div>
          <h4 className="font-display font-black text-[#1a1c1e] text-sm">{title}</h4>
          <p className="text-xs text-gray-500 font-medium leading-tight mt-1">{body}</p>
        </div>
      </div>
    ))}
  </div>
        ...
        {activeModal && (
          <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              onClick={closeModal} className="absolute inset-0 bg-[#1a1c1e]/80 backdrop-blur-md" />
                ...
                <div className="flex flex-col h-full max-h-[85vh]">
                  ...
                  <div className="space-y-6 overflow-y-auto pr-2 pb-4 flex-1 custom-scrollbar">
                    ...
                    <div className="space-y-2">
                      ...
                      <input type="text" placeholder="Ex: Base de données, ReactJS..."
                        value={newCours.libelle}
                        onChange={e => setNewCours({...newCours, libelle: e.target.value})}
                        className="w-full bg-[#f1f4f2] rounded-2xl py-5 px-6 font-bold text-lg outline-none border-2 border-transparent focus:border-[#006c49]/30" />
                    </div>

                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-2">Spécialité concernée</label>
                      ...
                      <div className="space-y-2 max-h-[240px] overflow-y-auto pr-1 custom-scrollbar">
                        {assignations.map((a, i) => (
                          <motion.div key={i} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
                            ...
                            <select value={a.typeSeance}
                              onChange={e => updateAssignation(i, 'typeSeance', e.target.value)}
                            ...
                            <select value={a.profId}
                              onChange={e => updateAssignation(i, 'profId', e.target.value)}
                  ...
                  <div className="pt-4 shrink-0 mt-auto bg-white">
                    <button onClick={handleAddCours} disabled={loading || !newCours.libelle.trim()}
                      ...
                      {loading ? <><Loader2 size={16} className="animate-spin"/> Création...</> : `Créer le module${assignations.length > 0 ? ` avec ${assignations.length} assignation(s)` : ''}`}
                    </button>
                  </div>
                </div>
              )}
                ...
                <div className="flex flex-col h-full max-h-[85vh]">
                  ...
                  <div className="space-y-6 overflow-y-auto pr-2 pb-4 flex-1 custom-scrollbar">
                    <div className="bg-[#f1f4f2] rounded-2xl px-5 py-4 space-y-1">
                      <p className="text-[10px] font-black font-display uppercase tracking-widest text-gray-400 mb-2">
                    ...
                    {importResult && (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                        ...
                        <button onClick={closeModal}
                          className="w-full py-5 bg-[#1a1c1e] text-white rounded-[2.5rem] font-display font-black
                                     text-xs uppercase tracking-[0.2em] hover:bg-[#006c49] transition-all mt-4">
                          Fermer
                        </button>
                      </motion.div>
                    )}
                  ...
                  {!importResult && (
                    <div className="pt-4 shrink-0 mt-auto bg-white">
                      <button onClick={handleImportSubmit} disabled={!csvFile || loading}
                        className="w-full py-6 bg-[#1a1c1e] text-white rounded-[2.5rem] font-display font-black text-xs
                                   uppercase tracking-[0.2em] hover:bg-[#006c49] disabled:bg-gray-200
                                   disabled:cursor-not-allowed transition-all flex items-center justify-center gap-3">
