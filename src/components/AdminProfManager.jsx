...
import {
  ...
  FileText, Mail, BarChart2, ArrowUpDown, Trash2, AlertCircle
} from 'lucide-react';
...
// ─────────────────────────────────────────────────────────────────────────────
// Skeleton — placeholder affiché pendant que les stats de présence chargent
// Remplace le badge "%" dans la carte avec une animation de pulsation
// ─────────────────────────────────────────────────────────────────────────────
const StatSkeleton = () => (
...
// ─────────────────────────────────────────────────────────────────────────────
// Modal détail prof — inchangée par rapport à l'original
// ─────────────────────────────────────────────────────────────────────────────
            ...
            <div ref={scrollRef} onScroll={handleScroll}
              ...
              <div className="w-full shrink-0 snap-center px-5 md:px-6 pb-6 space-y-4 overflow-y-auto custom-scrollbar h-full">
                ...
                <p className="text-[9px] font-black font-display uppercase tracking-widest text-gray-400">
                  Présence par module enseigné
                </p>

                {prof.statsChargees && prof.modulesStats?.length > 0 ? (
                  <div className="space-y-2">
                    {prof.modulesStats.map((ms, idx) => (
                      <div key={idx}
                        className="bg-[#f1f4f2] p-3 rounded-xl flex items-center justify-between gap-3">
                        <span className="text-sm font-bold text-[#1a1c1e] break-words
                                         w-full md:w-auto md:max-w-[150px]">
                          {ms.libelle}
                        </span>
                        <div className="flex items-center gap-3 shrink-0 w-1/2 justify-end">
                          <div className="flex-1 h-1.5 bg-white rounded-full overflow-hidden">
                            <div className="h-full bg-[#006c49] rounded-full"
                              style={{ width: `${ms.taux}%` }} />
                          </div>
                  ...
                  <div className="space-y-2">
                    {[1, 2, 3].map(i => (
                      <div key={i} className="bg-[#f1f4f2] p-3 rounded-xl flex items-center justify-between gap-3 animate-pulse">
...
// ─────────────────────────────────────────────────────────────────────────────
// Composant principal AdminProfManager
// ─────────────────────────────────────────────────────────────────────────────
const AdminProfManager = () => {
  ...
  // profToDelete   : le prof qu'on est en train de vouloir supprimer
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // PHASE 1 — Chargement rapide des infos de base
  //
  // On appelle /admin/tous-sans-stats qui retourne immédiatement
  // les données légères (nom, email, heures, modules, justifs).
  // Le taux de présence est null → les cartes s'affichent avec un skeleton.
  // ─────────────────────────────────────────────────────────────────────────
  const fetchProfs = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/api/professeurs/admin/tous-sans-stats`, { headers });
      if (!res.ok) return;
      const data = await res.json();
      setProfs(data);
      setLoading(false);

      // ── PHASE 2 — Chargement des stats de présence prof par prof ───────────
      //
      // Maintenant que les cartes sont affichées, on lance UN appel par prof
      // en parallèle (Promise.allSettled) pour récupérer son taux de présence.
      //
      // Promise.allSettled (et non Promise.all) :
      //   → si un prof échoue, les autres continuent quand même
      //   → on ne bloque pas tout à cause d'un seul prof problématique
      //
      // Pour chaque résultat reçu, on met à jour UNIQUEMENT la carte de ce prof
      // via setProfs(prev => prev.map(...)) → les autres cartes ne re-rendent pas.
      //
      await Promise.allSettled(
        data.map(async (prof) => {
          try {
            const statsRes = await fetch(
              `${API}/api/professeurs/${prof.id}/stats-presence`,
              { headers }
            );
            if (!statsRes.ok) return;
            const stats = await statsRes.json();

            // Mise à jour ciblée : on ne modifie que le prof concerné dans la liste
            setProfs(prev =>
              prev.map(p =>
                p.id === prof.id
                  ? {
                      ...p,
                      tauxPresenceGlobal: stats.tauxPresenceGlobal,
                      modulesStats:       stats.modulesStats,
                      statsChargees:      true,
                    }
                  : p
              )
            ...
            // Si le calcul échoue pour ce prof, on marque quand même comme chargé
            // avec des valeurs par défaut pour éviter le skeleton infini
            setProfs(prev =>
              prev.map(p =>
                p.id === prof.id
                  ? { ...p, tauxPresenceGlobal: 0, modulesStats: [], statsChargees: true }
                  : p
              )
            );
          }
  ...
  // Tri — si les stats ne sont pas encore chargées, on trie sur 0 pour "présence"
  filtered.sort((a, b) => {
    let valA, valB;
    if (sortBy === 'nom') {
  ...
  // Étape 1 : clic sur la poubelle → stocke le prof dans profToDelete
  //           → ouvre la modale de confirmation
  const handleDeleteClick = (e, prof) => {
    // stopPropagation OBLIGATOIRE : sans ça, le clic ouvrirait aussi la modale de détail
    e.stopPropagation();
    setProfToDelete(prof);
    setDeleteError(null);
  };

  // Étape 2 : confirmation → envoie DELETE au backend
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/professeurs/${profToDelete.id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      ...
      if (data.success) {
        // Retire le prof de la liste locale sans refetch → UI instantanée
  ...
  // Étape 3 : annulation → ferme la modale sans rien faire
  ...
  return (
    <div className="min-h-screen bg-[#f1f4f2] pt-24 pb-32 px-4 md:px-8 font-body relative overflow-hidden">
          ...
          <div className="flex flex-wrap gap-3 items-center">
            <div className="flex items-center gap-1 bg-white border-2 border-gray-200 p-1 rounded-[1.5rem] shadow-sm shrink-0">
              {['Tous', 'Actif', 'Inactif'].map(c => (
                <button key={c} onClick={() => setFilterCharge(c)}
            ...
            <button onClick={() => setFilterJustifs(p => !p)}
              className={`px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest
          ...
          <>
                ...
                {filtered.map((prof, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* ── BOUTON POUBELLE ──────────────────────────────────────
                          Positionné en haut à droite, visible seulement au hover.
                          z-30 pour être au-dessus du fond déco (z-0) et de la flèche (z-20).
                          e.stopPropagation() empêche l'ouverture de la modale de détail.
                      ─────────────────────────────────────────────────────────── */}
                      <button
                        onClick={(e) => handleDeleteClick(e, prof)}
                      ...
                      <div className="relative z-10 space-y-4">
                        ...
                        <div className="flex justify-between items-start">
                          ...
                          <div className="flex flex-col items-end gap-2">
                            {/* Badge taux de présence — skeleton si pas encore chargé */}
                            <div className="bg-[#f1f4f2] px-2.5 py-1 rounded-full border border-gray-100
                                            shadow-sm flex items-center gap-1 min-w-[60px] justify-center">
                          ...
                          <div className="pr-10">
                            ...
                            <div className="hidden md:flex flex-wrap gap-1.5">
                              ...
                              {prof.modules.length > 2 && (
                                <span className="px-2.5 py-1 bg-[#f1f4f2] text-gray-400 rounded-xl text-[9px] font-black">
                                  +{prof.modules.length - 2}
                                </span>
                              )}
                            </div>
                        ...
                        {/* Stats mini (heures / séances / justifs) */}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
              {[
                ...
                { label: 'Heures totales',    val: filtered.reduce((acc, p) => acc + (p.heures || 0), 0) + 'h',      color: 'text-[#006c49]' },
                { label: 'Séances effectuées',val: filtered.reduce((acc, p) => acc + (p.seancesTerminees || 0), 0),  color: 'text-[#1a1c1e]' },
                { label: 'Justifs en attente',val: filtered.reduce((acc, p) => acc + (p.justifsAttente || 0), 0),    color: 'text-orange-500' },
              ].map(({ label, val, color }) => (
      ...
      {/* ── Modale de confirmation de suppression ─────────────────────────
          S'affiche quand profToDelete n'est pas null.
            ...
            >
              ...
              <h2 className="font-display font-black text-3xl text-[#1a1c1e] tracking-tighter text-center leading-tight mb-2">
                ...
                <span className="text-red-500">
                  {profToDelete.prenom} {profToDelete.nom}
                </span>
                {' '}?
              </h2>
              ...
              <p className="text-[#006c49] font-black text-[10px] text-center uppercase tracking-widest mb-8">
                Ses séances et historiques restent conservés.
              </p>
